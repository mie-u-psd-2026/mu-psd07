...
| 応答速度 | AIリクエスト後、計画・生成・審査を行う間は処理中表示を出す。サーバー全体180秒、画面190秒でタイムアウト |
| 可読性 | 質問と選択肢を明確に区別する |
...
AIへの指示は `backend/planning_prompt.txt`（計画）、`backend/prompt.txt`（質問生成）、`backend/result_prompt.txt`（結果生成）、`backend/review_prompt.txt`（審査共通）、`backend/question_review_prompt.txt`と`backend/result_review_prompt.txt`（対象別の審査基準）で管理する。依存関係は `backend/requirements.txt`。
バックエンドの `decision.py` が計画・生成・意味の審査を別々のAI呼び出しで行う。PythonはJSON形式、根拠の原文引用、質問と計画の対応、同一文の再質問、選択肢の形式、推薦候補の一致を検証する。通常モードでは犬・猫の習性を利用者に尋ねるなど、明白な不適切質問を軽量なチェックでも検出する。審査は補助であり、意味の正しさを完全に保証するものではない。
...
通常は1回のAI呼び出しで計画を先に出力してから選択式の質問・結論を生成する。修正は1回まで。生成時にはquestion_examples.jsonから関連する例を最大2件参考にする。別のAIによる審査はDECISION_STRICT_REVIEW=trueの検証用モードで使う。
...
2. **生成**：選んだ条件について初心者が答えられる1問を作る。内部のpurposeで判断への用途を説明する。金額や時間も範囲で足りるなら選択式とし、数値・自由入力が必要な場合だけinput_reasonで理由を示す。
3. **審査（厳格モードのみ）**：質問用と結果用のスキーマ・基準を分け、計画の根拠、相談との関連、未確認情報か、答えやすさ、中立性、選択肢の適切さ、選択式優先、結果の根拠、候補比較、不確実性を意味で確認する。

JSON不正や形式検証の失敗は最大3回まで修正する。厳格モードでは意味の審査不合格も修正対象にする。質問は計画内の別の未確認条件を優先して試し、他に条件がなければ再計画する。結果の不合格は再計画する。通常モードでは意味の審査を必須にしない。厳格モードでは最終試行や途中終了も意味の審査を省略しない。全体180秒を超えた場合や品質を確認できない場合は再試行可能なエラーを返す。固定質問へ置き換えたり、再生成回数だけで結果に切り替えたりしない。

計画と審査はサーバー内に留め、既存の質問・結果JSONだけを画面へ返す。スキップ文面は現在の相談内で保持し、別の相談を一律に禁止しない。従来のskip_log.jsonへの記録は残すがモデルの学習や全相談共通の禁止リストとしては使わない。
