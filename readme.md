...
質問用AIは `qwen3:4b-instruct-2507-q4_K_M`、結論用は `qwen3:8b` を使用します。モデルの詳細は [Ollama公式ページ](https://ollama.com/library/qwen3:8b) を参照してください。
...
既定の軽量モデルは通常モードで使います。速度を優先し、通常は推論モードを無効にしています。

通常は1回のモデル呼び出しの中で条件整理を先に出力し、その後に選択式の質問または結論を生成します。質問だけなら不正な応答の修正を含め最大2回です。軽量モデルが自然終了を判断した場合は結論モデルへ引き継ぎます。結論用モデルはOLLAMA_RESULT_MODELで変更できます（既定8B）。`DECISION_STRICT_REVIEW=true`で、従来の意味審査も必須にする検証用モードを有効にできます。修正を含め1操作全体で180秒、画面では190秒の期限があります。遅い端末では`DECISION_TIMEOUT_SECONDS`を30〜600秒の範囲で設定でき、画面の期限もサーバーの設定に合わせて10秒長くなります。応答の形式が不正な場合や通信エラーでは入力を保持して再試行できます。履歴を収めるため文脈長は8192で、環境変数`OLLAMA_NUM_CTX`で調整できます。
...
質問の例が合う場合はAIが番号を選び、質問文と選択肢をサーバーが補います。例が合わない相談では選択式質問を生成します。短い指示と出力を使い、結論のみ8Bに任せます。平均20秒は目標であり、端末や履歴による速度差があります。

対象のノートPCで `python -m backend.benchmark_latency --cpu --output evaluation-results/laptop.json` を実行すると質問・スキップ・例のない相談・途中結論の5件を計測します。自然終了時の追加呼び出しや長い履歴は別途測定が必要です。GPUを使う測定では `--cpu` を外します。

### CPUでの代理測定（2026-09-11）

Ryzen 7 5700XでGPUを無効にした測定は、質問4件が平均11.331秒、別測定の8B結論57.470秒を合わせた5件平均20.559秒でした。記録: evaluation-results/laptop-mixed-cpu-final.json。ノートPC実測ではなく、自然終了時の引き継ぎ・長い履歴を含む平均でもありません。平均20秒以下の達成保証はせず、対象端末で測定してください。結論には一般化が残る場合があり、速度改善を精度保証と解釈しないでください。自動テスト55件と画面状態テスト通過。
